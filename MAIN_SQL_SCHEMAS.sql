...
WITH city_table AS (
    SELECT 
        ...
        COUNT(DISTINCT s.customer_id) AS total_cx,
        ROUND(
            CAST(SUM(s.total) AS DECIMAL(15,2)) / CAST(COUNT(DISTINCT s.customer_id) AS DECIMAL(15,2)),
            2
        ) AS avg_sale_pr_cx
    FROM sales AS s
...
SELECT 
    ...
    ct.avg_sale_pr_cx,
    ROUND(
        CAST(cr.estimated_rent AS DECIMAL(15,2)) / CAST(ct.total_cx AS DECIMAL(15,2)),
        2
    ) AS avg_rent_per_cx
